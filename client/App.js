...
export default function App() {
  // const socket = socketIO.connect("http://192.168.104.12:3000");
  ...
  useEffect(() => {
    if (user) {
      ...
      for (let i = 0; i < user.contactList.length; i++) {
        axios
          .get(`http://192.168.104.12:5001/users/${user.contactList[i]}`)
          .then((res) => {
      ...
      for (let i = 0; i < user.pendingRequests.length; i++) {
        axios
          .get(`http://192.168.104.12:5001/users/${user.pendingRequests[i]}`)
          .then((res) => {
